			...
			try{
				...
				while(resultSet.next()) {
					...
					if(quantity>=pQuantity)
					{
						...
						PreparedStatement st = connection.prepareStatement("UPDATE MeatProductList SET quantity = ? WHERE productname = ?");
		...
		else if(userType==1)
		{
			...
			try {
				...
				if (!resultSet.isBeforeFirst()) {
					...
					String confirmMsg = scanner.next();
					if (confirmMsg.equalsIgnoreCase("Yes")) {
						PreparedStatement stmt = connection.prepareStatement("INSERT INTO MeatProductList VALUES (?, ?)");
					...
					ResultSet resultSet1=statement.executeQuery("select * from MeatProductList where productname= '" + pName + "'");
					int oldQuantity=0;
